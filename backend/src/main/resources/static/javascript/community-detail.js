...
$(document).ready(async function () {
    ...
    $("#commentWriteBtn").on("click", createComment);
});
...
async function loadPostDetail() {
    ...
    if (currentUser && (currentUser.username === post.authorUsername || currentUser.role === "ADMIN")) {
        $("#editPostBtn").show().click(() => location.href = `community-edit.html?postId=${post.id}`);
        $("#deletePostBtn").show().click(() => deletePost(post.id));
    }
}
...
async function deletePost(postId) {
    ...
    if (res.ok) location.href = "/community.html";
    else alert("삭제 실패");
}
